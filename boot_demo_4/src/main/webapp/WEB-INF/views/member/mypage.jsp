                ...
                <div class="col-xl-9">
                    ...
                    <div class="card shadow-sm p-4">
                        ...
                        <c:if test="${not empty member.cartList}">
                            <div class="section-title"><i class="bi bi-card-list me-2"></i>구매 이력</div>
                            <div class="card border shadow-sm overflow-hidden">
                                <table class="table table-hover align-middle mb-0">
                                    <thead class="table-light text-secondary">
                                        <tr>
                                            <th class="ps-4">구매날짜</th>
                                            <th>상품명</th>
                                            <th class="text-center">구매수량</th>
                                            <th class="text-end pe-4">상품별 구매금액</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <c:forEach items="${member.cartList}" var="cart">
                                            <c:set var="prod" value="${cart.prod}"/>
                                            <c:set var="totalPrice" value="${totalPrice + (prod.prodPrice * cart.cartQty)}"/>
                                            <tr>
                                                <td class="ps-4 text-muted">${cart.cartDate}</td>
                                                <td><span class="fw-bold text-dark">${prod.prodName}</span></td>
                                                <td class="text-center">
                                                    <span class="badge bg-light text-dark border">${cart.cartQty}</span>
                                                </td>
                                                <td class="text-end pe-4 fw-bold text-primary">
                                                    <fmt:formatNumber value="${prod.prodPrice * cart.cartQty}" pattern="#,###" />원
                                                </td>
                                            </tr>
                                        </c:forEach>
                                        <tr class="table-primary border-top border-2">
                                            <td colspan="3" class="text-end fw-bold text-dark ps-4">구매 총액</td>
                                            <td class="text-end pe-4 fs-5 fw-bold text-primary">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="gap-2 d-flex">
                                    ...
                                    <a href="/member/change-password" class="btn btn-warning px-4 fw-bold text-white">비밀번호 변경</a>
